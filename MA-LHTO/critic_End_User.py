            ...
            for dev in range(config.get('Dev_dev')):
                ...
                for task_number in range(len(task_queue[dev][i])):
                    if x[dev][task_number] > 0:
                        ...
                        sub_task = info.loc[(info.index == task_queue[dev][i][task_number][0])]
                        if sub_task['gpu_spec'].values[0] == -1:
                            info.loc[info.index == task_queue[dev][i][task_number][0], 'mes_cpu'] = f_mes[dev][task_number][0]* F_es[0][0]
                            ...
                            info.loc[info.index == task_queue[dev][i][task_number][0], 'mes_cpu'] = f_mes[dev][task_number][0] * F_es[sub_task['gpu_spec'].values[0] + 1][0]
                            info.loc[info.index == task_queue[dev][i][task_number][0], 'mes_gpu'] = f_mes[dev][task_number][1] * 4
        # ④是进入计算队列还是卸载队列
        for dev in range(config.get('Dev_dev')):
