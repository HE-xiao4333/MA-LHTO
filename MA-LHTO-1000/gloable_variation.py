...
F_eu=[96000/2,4/2] #CPU,GPU, G2 96000 786432
#######################################################################################################
...
F_es=[[96000*3*12,0]]
for i in range(2):
    F_es.append([(info_GPU.iloc[i]['cpu_milli']*12*3),(info_GPU.iloc[i]['gpu']*12*3)])

for edge in range(config.get('Dev_edge')):
    ES_lasting_res[edge][0][0].append([96000*12]+[0]*50)
    ES_lasting_res[edge][0][0].append([96000*12]+[0]*50)
    ES_lasting_res[edge][0][0].append([96000*12]+[0]*50)
    for k in range(2):
        ES_lasting_res[edge][k+1][0].append([info_GPU.iloc[k]['cpu_milli']*12]+[1]*50)
        ES_lasting_res[edge][k+1][0].append([info_GPU.iloc[k]['cpu_milli']*12]+[1]*50)
        ES_lasting_res[edge][k+1][0].append([info_GPU.iloc[k]['cpu_milli']*12]+[1]*50)
